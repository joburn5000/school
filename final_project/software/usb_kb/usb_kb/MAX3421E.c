...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	wdata[0] = reg + 2;
	wdata[1] = val;
	int return_code = alt_avalon_spi_command(0x000000c0, 0, 2, wdata, 0, NULL, 0);
	if(return_code < 0) {
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int return_code = alt_avalon_spi_command(0x000000c0, 0, nbytes+1, wdata, 0, NULL, 0);
	if(return_code < 0) {
...
BYTE MAXreg_rd(BYTE reg) {
	...
	alt_u8 rdata[1];
	wdata[0] = reg;
	int return_code = alt_avalon_spi_command(0x000000c0, 0, 1, wdata, 1, rdata, 0);
	if(return_code < 1) {
